/************************************************************************
 * QUESTLOG — Google Apps Script backend
 * Connects your Google Sheet to the QuestLog website.
 *
 * SETUP (full walkthrough in SETUP.md):
 *   1. Fill in CLIENT_ID and ALLOWED_EMAILS below.
 *   2. Run "QuestLog > Set up sheets" from the menu (reload the sheet first).
 *   3. Run "QuestLog > Import starter games" once to load Lynn's list.
 *   4. Run "QuestLog > Refresh Steam data" to pull reviews + trailers.
 *   5. Deploy > New deployment > Web app > Execute as: Me, Access: Anyone.
 *   6. Copy the /exec URL into the website's CONFIG.APPS_SCRIPT_URL.
 ************************************************************************/
...
const SHEETS = {
  Games:    ["appid","title","people","genres","modes","dim","minPlayers","maxPlayers","mods","note","reviewPct","reviewDesc","releaseDate","releaseTs","trailer","addedBy","addedTs"],
...
/* ---------- menu ---------- */
function onOpen(){
  SpreadsheetApp.getUi().createMenu("QuestLog")
...
function doGet(e){
  try{
    ...
    const games=rows("Games").map(g=>{
      // graceful fallback: if an old sheet only had a single "players" column, use it as max
      const maxP = Number(g.maxPlayers) || Number(g.players) || 4;
      const minP = Number(g.minPlayers) || 1;
      return {
...
function doPost(e){
  try{
    ...
    if(a==="addGame"){
      ...
      const now=Date.now();
      let minP=Number(g.minPlayers)||1, maxP=Number(g.maxPlayers)||1; if(minP>maxP){ const t=minP; minP=maxP; maxP=t; }
      const game={ appid, title:g.title||steam.title||("App "+appid), people:(g.people||[]).join(","),
        ...
        minPlayers:minP, maxPlayers:maxP, mods:!!g.mods, note:g.note||"",
...
/* ---------- auth (Google OAuth2 access token) ----------
   The site uses the GIS token flow and sends an access token.
   We verify it with Google's tokeninfo endpoint, check it was issued
   for OUR client, and confirm the email is on the guest list.        */
function verify(token){
  ...
  try{
    ...
    if(CLIENT_ID && p.aud!==CLIENT_ID && p.azp!==CLIENT_ID) return null;
    const email=String(p.email||"").toLowerCase();
    if(!email) return null;
    if(ALLOWED_EMAILS.length && ALLOWED_EMAILS.indexOf(email)===-1) return null;
    return {email, name:p.name||"", picture:""};
...
function importStarter(){
  ...
  seed.forEach((g,idx)=>{
    ...
    const row={ appid, title:g.title||steam.title||("App "+appid), people:(g.people||[]).join(","),
      genres:(g.genres||[]).join(","), modes:(g.modes||[]).join(","), dim:g.dim||"2D",
      minPlayers:g.minPlayers||1, maxPlayers:g.maxPlayers||1, mods:!!g.mods, note:g.note||"",
      reviewPct:steam.reviewPct, reviewDesc:steam.reviewDesc, releaseDate:steam.releaseDate,
